...
叠加层（MessageBox、Dialog）在拖拽时启用 `set_layer_enabled` 和子树缓存，将重绘限制在有损图层内，避免触发宿主窗口的全帧重绘。拖拽 delta 变化低于阈值（< 0.01px）时直接跳过 transform 提交和后续重绘，消除高频指针事件的无效屏幕更新。

拖拽期间宿主视口在指针 Down 时快照缓存，避免每帧重复调用布局系统的 `viewport_for()` 查询。
