...
class DxCompositionSurface final {
  public:
    enum class RenderResult { Rendered, Skipped, StaleTargetSize, DeviceLost };

    explicit DxCompositionSurface(HWND hwnd);
    DxCompositionSurface(HWND hwnd, std::shared_ptr<DxRenderDevice> device);
    ~DxCompositionSurface();

    DxCompositionSurface(const DxCompositionSurface&) = delete;
    DxCompositionSurface& operator=(const DxCompositionSurface&) = delete;
    DxCompositionSurface(DxCompositionSurface&&) noexcept;
    DxCompositionSurface& operator=(DxCompositionSurface&&) noexcept;
