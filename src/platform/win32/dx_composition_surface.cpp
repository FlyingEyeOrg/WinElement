#include "dx_composition_surface.hpp"
...
class DxCompositionSurface::Impl final {
  public:
    Impl(HWND hwnd, std::shared_ptr<DxRenderDevice> device)
  ...
  private:
    ...
    std::shared_ptr<DxRenderDevice> device_;
    ...
    DxRenderResourceCache resource_cache_;
    std::optional<PixelSize> validated_client_size_;
    bool device_recreated_ = false;
};

DxCompositionSurface::DxCompositionSurface(HWND hwnd)
    : DxCompositionSurface(hwnd, std::make_shared<DxRenderDevice>()) {}

DxCompositionSurface::DxCompositionSurface(HWND hwnd, std::shared_ptr<DxRenderDevice> device)
    : impl_(std::make_unique<Impl>(hwnd, std::move(device))) {}

DxCompositionSurface::~DxCompositionSurface() = default;

DxCompositionSurface::DxCompositionSurface(DxCompositionSurface&&) noexcept = default;

DxCompositionSurface& DxCompositionSurface::operator=(DxCompositionSurface&&) noexcept = default;

void DxCompositionSurface::set_dpi(float dpi) noexcept {
    impl_->set_dpi(dpi);
}

void DxCompositionSurface::invalidate_surface_size() noexcept {
    impl_->invalidate_surface_size();
}

void DxCompositionSurface::discard() noexcept {
    impl_->discard();
}

void DxCompositionSurface::upload_resource(rendering::RenderResourceUpload upload) noexcept {
    impl_->upload_resource(std::move(upload));
}

DxCompositionSurface::RenderResult DxCompositionSurface::render(
...
bool DxCompositionSurface::consume_device_recreated() noexcept {
