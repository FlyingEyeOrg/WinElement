...
[[nodiscard]] winelement::rendering::RenderScene build_probe_scene() {
    ...
    backdrop.configure_layout([](winelement::layout::LayoutElement& item) {
        item.set_position_type(winelement::layout::PositionType::Absolute)
            .set_position(winelement::layout::Edge::Left,
                          winelement::layout::Length::points(680.0F))
            .set_position(winelement::layout::Edge::Top,
                          winelement::layout::Length::points(152.0F))
            .set_width(winelement::layout::Length::points(300.0F));
    });
    for (const auto label : {std::string_view{"Open dialog"}, std::string_view{"Non-modal dialog"}}) {
    ...
    winelement::controls::Dialog::show(
        root, winelement::controls::DialogOptions{
                  .title = "Dialog",
                  .body =
                      "Modal surface with header, body, footer, close and confirm actions.",
...
int main(int argc, char** argv) {
    try {
        const auto output_path =
            argc > 1 ? fs::path(argv[1]) : fs::current_path() / "dialog_spacing_probe.png";
        std::uint32_t stride = 0U;
        const auto pixels = render_probe(stride);
        save_png(output_path, probe_width, probe_height, stride, pixels);
        fmt::print("dialog_spacing_probe output: {}\n", output_path.string());
        return 0;
    } catch (const std::exception& exception) {
        fmt::print(stderr, "dialog_spacing_probe failed: {}\n", exception.what());
        return 1;
    }
}
